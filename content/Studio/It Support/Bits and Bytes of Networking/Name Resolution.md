[[Studio/It Support/Bits and Bytes of Networking/Servizi di Rete]]
#reti 
# Name Resolution
>[!note]- ICANN
...
>[!note] Registrar
>Compagnia che ha un accordo con ICANN per vendere [[Studio/It Support/Bits and Bytes of Networking/Name Resolution#Domain name nome di dominio|nomi dominio]] ancora non registrati
...
Termine per qualcosa che può essere "risolto" (tradotto) dal [[Studio/It Support/Bits and Bytes of Networking/Name Resolution#DNS Domain Name System|DNS]], costruito da una serie di stringhe (testo) separate da punti.
...
Usando il [[Studio/It Support/Bits and Bytes of Networking/Name Resolution#DNS Domain Name System|DNS]] un organizzazione o un individuo può cambiare in quale IP il proprio [[Studio/It Support/Bits and Bytes of Networking/Name Resolution#Domain name nome di domini|dominio]]
...
DNS può supportare fino a 127 livelli di dominio per il totale di un singolo [[Studio/It Support/Bits and Bytes of Networking/Name Resolution#Fully Qualified Domain Name FQDN|FQDN]] con al massimo 255 caratteri totali, ogni sezione dominio può avere massimo 63 caratteri
...
![[Studio/Excalidraw/FQDN.excalidraw]]
Un nome a dominio pienamente qualificato ([[Studio/It Support/Bits and Bytes of Networking/Name Resolution#Fully Qualified Domain Name FQDN|FQDN]]) si distingue per la sua mancanza di ambiguità: può essere interpretato solo in un modo
...
Chiamato generalmente dominio, è "figlio" del [[Studio/It Support/Bits and Bytes of Networking/Name Resolution#TLD Top Level Domain|TLD]], ovvero è la parte che lo precede. (sottodominio del [[Studio/It Support/Bits and Bytes of Networking/Name Resolution#TLD Top Level Domain|TLD]]), "padre" del sottodominio.
...
### TTL
Tutti i nomi di dominio nel sistema DNS globale hanno un TTL, valore in secondi configurabile dal proprietario del dominio, di quanto a lungo un server (ad esempio un [[Studio/It Support/Bits and Bytes of Networking/Name Resolution#^t0l6w4|server cache]]) possa tenere in cache la traduzione domino=>IP prima di effettuare di nuova una resolution name request e re-immagazzinare l'associazione dominio=IP.

### Tipi di server DNS
Ci sono principalmente 5 tipi di server DNS

1. **Caching Name Resolver**: generalmente forniti dagli ISP. il loro scopo è di immagazzinare un [[Studio/It Support/Bits and Bytes of Networking/Name Resolution#Domain name nome di dominio|nome dominio]] conosciuto per un periodo di tempo, permette di tenere pronti i nomi più utilizzati senza ogni volta avviare una ricerca  ^t0l6w4
...
- Step da parte del Caching Name resolver/Recursive Name Server
	1. Contattare [[Studio/It Support/Bits and Bytes of Networking/Name Resolution#^frmrvu|root name server]]
	2. Risposta con quale [[Studio/It Support/Bits and Bytes of Networking/Name Resolution#^fd92gq|TLD name server]] da contattare (quale TLD interrogare)
	3. Query [[Studio/It Support/Bits and Bytes of Networking/Name Resolution#^fd92gq|TLD name server]], per ogni TLD in esistenza esiste "un" suo TLD server
	4. Risposta da parte del TLD con il nome dell'[[Studio/It Support/Bits and Bytes of Networking/Name Resolution#^a6447k|Authoritative server]] da contattare
	5. Query delle ultime parti del nome dominio dell'[[Studio/It Support/Bits and Bytes of Networking/Name Resolution#^a6447k|Authoritative server]] 
	6. Risposta da parte dell'[[Studio/It Support/Bits and Bytes of Networking/Name Resolution#^a6447k|Authoritative server]] con l'indirizzo IP associato al dominio
	7. Immagazzinare nella cache del [[Studio/It Support/Bits and Bytes of Networking/Name Resolution#^t0l6w4|server cache]] l'abbinamento IP e dominio (alla scadenza in caso di richiesta DNS il processo dovrà essere ripetuto da capo)
...
>	4. .org TLD: devi contattare il [[Studio/It Support/Bits and Bytes of Networking/Name Resolution#^a6447k|Authoritative server]] di www.wikipedia
>	5. Caching Name server/Recursive Name Server: [[Studio/It Support/Bits and Bytes of Networking/Name Resolution#^a6447k|Authoritative server]] di www.wikipedia  che indirizzo IP ha www.wikipedia.org??
>	6.  [[Studio/It Support/Bits and Bytes of Networking/Name Resolution#^a6447k|Authoritative server]]  di www.wikipedia: controllo i miei resource records di www.wikipedia.com ........ che ha come indirizzo IP 208.80.154.224
...
#### A Record
Usato per associare un determinato domain name a un determinato IP address. Nel suo uso più basico è configurato per un nome di dominio singolo, ma è possibile per un singolo nome di dominio avere multipli **A Record**. Questo gli permette di usare una tecnica chiamata [[Studio/It Support/Bits and Bytes of Networking/DNS Round Robin]] 
#inserire_immagine 
#### AAAA Record (Quad A)
Molto simile all'[[Studio/It Support/Bits and Bytes of Networking/Name Resolution#A Record|A record]] solo che restituisce un indirizzo IPv6 invece che IPv4
#inserire_immagine 
...
#inserire_immagine 
#### SRV Record (Service)
Simile al [[Studio/It Support/Bits and Bytes of Networking/Name Resolution#MX Record Mail eXchange|MX record]] usato per definire la posizione di servizi specifici. Ha lo stesso scopo del server MX eccetto per un caso; mentre [[Studio/It Support/Bits and Bytes of Networking/Name Resolution#MX Record Mail eXchange|MX record]] è solo per servizi mail un SRV record può essere definito per restituire le specifiche di molti tipi di servizi diversi
...
Da un IP risolve un nome dominio, grazie al [[Studio/It Support/Bits and Bytes of Networking/Name Resolution#Reverse Lookup Zone File|Reverse Lookup Zone File]]
...
### DNS Zones
Concetto gerarchico in cui uno specificato server è responsabile per una [[Studio/It Support/Bits and Bytes of Networking/Name Resolution#DNS Zones|Zona DNS]] 
| Server dns                                             | Zona responsabilità                                                  |
| ------------------------------------------------------ | -------------------------------------------------------------------- |
| [[Studio/It Support/Bits and Bytes of Networking/Name Resolution#^frmrvu\|root name server]]          | root zone                                                            |
| [[Studio/It Support/Bits and Bytes of Networking/Name Resolution#^fd92gq\|TLD name server]]           | ogni TLD è responsabile per la sua specifica zona (.org, .com, .etc) |
| [[Studio/It Support/Bits and Bytes of Networking/Name Resolution#^a6447k\|Authoritative name server]] | Responsabile di zone più dettagliate al disotto, ognuno avrà il suo dominio e sottodomini (www.google,it.wikipedia)                      |                                                                     |
...
![[Studio/It Support/Bits and Bytes of Networking/Name Resolution#SoA Record Start of Authority]]

##### Reverse Lookup Zone File
Permettono ai DNS resolver di fornire un indirizzo IP e ottenere il [[Studio/It Support/Bits and Bytes of Networking/Name Resolution#Fully Qualified Domain Name FQDN|FQDN]] di un dominio




### DNS e UDP
DNS è un ottimo esempio di un servizio dell'[[Studio/It Support/Bits and Bytes of Networking/Application Layer]] che nel [[Studio/It Support/Bits and Bytes of Networking/Transport Layer]] utilizza il protocollo [[Studio/It Support/Bits and Bytes of Networking/Transport Layer#Connectionless protocols|UDP]] invece che TCP

Una singola [[Studio/It Support/Bits and Bytes of Networking/Name Resolution#Full Resolution Name Request DNS Lookup DNS Request|richiesta DNS]] e la sua risposta può solitamente essere contenuta in un singolo datagramma UDP, rendendola la candidata ideale per tale protocollo.

Vale anche la pena dire che il DNS può generare un sacco di traffico

L'UDP è ideale anche perché un [[Studio/It Support/Bits and Bytes of Networking/Name Resolution#Tipi di server DNS|server DNS]] non deve fare nulla solo rispondere alle richieste di lookup in arrivo.

Un [[Studio/It Support/Bits and Bytes of Networking/Name Resolution#^j1msyp|DNS resolver]] ha il semplice bisogno di effettuare lookups e ripeterli se non hanno successo.
...
## DNS as a Service
![[Studio/It Support/Bits and Bytes of Networking/Name Resolution#^al2xn7]]
### Public DNS Server
Name server specificatamente impostati in modo che chiunque possa usarli, gratuitamente.

La maggioranze dei DNS pubblici risponde agli [[Studio/It Support/Bits and Bytes of Networking/Troubleshooting Networks#ICMP Internet Control Message Protocol|ICMP message]] 
